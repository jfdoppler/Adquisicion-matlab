...
%% Defining constants
carpeta_base='F:\Juan 2018\canto';
log_filename='AVIMAT-log.txt';
nombreave='Canario-gris'; %identificación del pájaro a grabar
save_vs=1; %1 to save the vS channel, 0 don't save
playback=0; %should the script do a nighttime playback protocol
playback_folder='F:\Juan 2018\Playbacks\Benteveo';
playback_time=[18 24, 0 8]; %hour range in which to do playbak protocol
playback_files=dir([playback_folder '\*.wav']); %file list of playback wavs
playback_inter_prot_delay_lambda=60*60; %mean value of seconds of delay between consecutive protocols
playback_record_time=40; %number of seconds to recall for each playback
...
random_sampling_prob=0.5; %probability of saving a random acquisition (used for bg estimation)

%trigger con sonido
tiempo_medir=60*60*24*60; %segundos totales que se va a medir
daytime=[8 21]; %hour range of daytime (revisar que timer este en hora con la PC) 
tiempo_file=60; %segundos de medida por cada trigger

vs_ch=3; %Analog input channel of vS signal
vs_InputRange=[-1,1]; %Voltage range of vS channel
vs_trigger=0.05; %triger voltage value for vs channel
vs_integral_threshold=100; %threshold of integrated abs value in a tiempo ventana
vs_saturation=1.1*vs_InputRange(2); %saturation level for vs channel

ekg_ch=2; %Analog input channel of vS signal
ekg_InputRange=[-1,1]; %Voltage range of vS channel
ekg_trigger=0.05; %triger voltage value for vs channel
ekg_integral_threshold=100; %threshold of integrated abs value in a tiempo ventana
ekg_saturation=1.1*vs_InputRange(2); %saturation level for vs channel

s_ch=0; %Analog input channel of sound signal
s_InputRange=[-1,1]; %Voltage range of sound channel
s_trigger=0.0; %triger voltage value for sound channel
s_integral_threshold= 0; %threshold of integrated abs value in a tiempo ventana
...
while(seguir) % Starting main loop 
   try %por si falla algo no se cuelga el script y sigue grabando
      ...
      ch = addchannel(AI, [vs_ch s_ch ekg_ch], {'vs','sound', 'ekg'});      % set two channels  1 and 2 on the board
      AI.Channel(1).InputRange=vs_InputRange; %setting input range of the channels
      AI.Channel(2).InputRange=s_InputRange;
      AI.Channel(3).InputRange=ekg_InputRange;
      ...
      while AI.SamplesAcquired < AI.SamplesPerTrigger
            ...
            is_vs_saturated=(sum(abs(data(:,1))>vs_saturation)>=1); %cheking that vS is not saturated
      ...
      if kk || random_save %if supra-threshold
         ...
         cuando=fix(clock);
         cuando(1)=cuando(1)+1; %fecha atrasada (licencia ML)
         ...
         ekg = channels(1:fin,3); %vs is the first channel added
         time=time(1:fin);
         ...
         name_ekg=[nombreave,'_',fecha,'_',hora,'_ekg_',num2str(int2str(n),'%03i')];
         name_ekg=[name_ekg '.wav'];
         filename_ekg=[carpeta '\' name_ekg]; %genera el nombre del archivo de sonido
         max_ekg=max(ekg);
         min_ekg=min(ekg);
         ...
         log=fopen([carpeta '\' log_filename],'at'); %creating log file in folder
          fprintf(log,'%s\t%s\t%s\t%s\t%s\t%s\t%s\t%f\t%f\t%f\t%f\t%s\t%s\t%f\t%s\n', ...
            name_s,name_vs,name_ekg,fecha,hora,max_s,min_s,max_vs,min_vs,max_ekg,min_ekg,trigger_type,playback_wav_current,playback_silence_delay,desc); %creating header
         fclose(log); 
         ...
         norm_ekg=1.998*(ekg-(max_ekg+min_ekg)/2.)/(max_ekg-min_ekg);
         wavwrite(norm_ekg,SampleRate,filename_ekg); % saves as .wav files
         if save_vs
         ...
         close
         figure(n)
         subplot(311),plot(time,sonido)
%          if save_vs
         subplot(312),plot(time,vs)
         subplot(313),plot(time,ekg)
%          else
%              [sS,F,T,P] = spectrogram(sonido,1024,512,1024,ActualRate);
%              subplot(212),surf(T,F,10*log10(abs(P)),'edgecolor','none'); axis tight;
%              view(0,90);
%              ylim([0 6000])
%          end
         drawnow
